...
const useCreditCardValidator = () => {
  ...
  function updateCardNumber(cardNum) {
    setCardNum(cardNum);
  ...
  function getSumEven(cardNum) {
    return cardNum
      .split("")
      .reverse()
      .filter((_, i) => i % 2 === 1)
      .map((s) => (s * 2).toString().split(""))
      .flat()
      .map((d) => parseInt(d))
      .reduce((acc, curr) => acc + curr, 0);
  }

  return [updateCardNumber, cardNum, isCorrect, provider];
};
